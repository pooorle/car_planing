...
class PathPlanning:
    ...
    def run(self):
        ...
        image_path = os.path.join(current_dir, "snippets/car_r_30.png")
        car_image = pygame.image.load(image_path)

        image_path1 = os.path.join(current_dir, "snippets/target_r_t.png")
        target_image = pygame.image.load(image_path1)

        image_path2 = os.path.join(current_dir, "snippets/target_g_t.png")
        target_image_g = pygame.image.load(image_path2)

        image_path3 = os.path.join(current_dir, "snippets/left_cone_s.png")
        left_cone_image = pygame.image.load(image_path3)

        image_path4 = os.path.join(current_dir, "snippets/right_cone_s.png")
        right_cone_image = pygame.image.load(image_path4)

        image_path5 = os.path.join(current_dir, "snippets/left_spline_s.png")
        left_spline_image = pygame.image.load(image_path5)

        image_path6 = os.path.join(current_dir, "snippets/right_spline_s.png")
        ...
        while not self.exit:
            ...
            if len(visible_left_cones) > 1 and car.auto is True and new_visible_left_cone_flag is True:
                ...
                tck, u = splprep([x, y], s=0, k=K)  # type: ignore
                unew = np.arange(0, 1.01, 0.25 / (len(x) ** 1.2))  # more cones  = less final var
                left_spline = splev(unew, tck)  # type: ignore
            ...
            if len(visible_right_cones) > 1 and car.auto is True and new_visible_right_cone_flag is True:
                ...
                tck, u = splprep([x, y], s=0, k=K)  # type: ignore
                unew = np.arange(0, 1.01, 0.25 / (len(x) ** 1.2))  # more cones  = less final var
                right_spline = splev(unew, tck)  # type: ignore
            ...
            if len(visible_left_cones) > 1 and len(visible_right_cones) > 1 and (
                ...
                if len(path_midpoints[0]) == 1:
                    ...
                    path_midpoints_spline = splev(unew, tck)  # type: ignore
                    ...
                    for i in range(len(path_midpoints_spline[0])):
                        ...
                        if new_target_loc in target_locations:
                            ...
                            for j in range(len(target_locations)):
                                if np.linalg.norm(
                                        tuple(x - y for x, y in zip(target_locations[j], new_target_loc))) < 1:
                                    make_target = False
                                    break
            ...
            for right_cone in right_cones:
                right_cone.update(car, ppu, car_angle)
